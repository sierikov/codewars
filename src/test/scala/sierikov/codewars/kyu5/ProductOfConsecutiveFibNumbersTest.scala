...
class ProductOfConsecutiveFibNumbersTest extends AnyFlatSpec {
  ...
  it should "calculate correctly using immutable" in {
    testData.map {
      case (input, expected) => ProductOfConsecutiveFibNumbers.find(input) shouldEqual expected
    }
  }

  it should "calculate correctly using mutable" in {
    testData.map {
      case (input, expected) => ProductOfConsecutiveFibNumbers.findMutable(input) shouldEqual expected
    }
  }
}
